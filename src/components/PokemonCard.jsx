...
function TypeBadge({ type, multiplier = null, accent = 'default' }) {
  const c = TYPE_COLORS[type] || { bg: '#44444420', border: '#44444460', text: '#aaaaaa' }
  ...
  return (
    ...
    >
      {type}
      {multiplier != null && <em className={styles.badgeMultiplier}>×{multiplier}</em>}
    </span>
  )
}
...
function EvoStage({ name, sprite, isCurrent, isLast, level }) {
  return (
    <div className={styles.evoRow}>
      <div className={`${styles.evoStage} ${isCurrent ? styles.evoCurrent : ''}`}>
        ...
        <span className={`${styles.evoCurBadge} ${!isCurrent ? styles.evoCurBadgePlaceholder : ''}`}>
...
export default function PokemonCard({ data }) {
  ...
  const [damageProfile, setDamageProfile] = useState({ more: [], normal: [], less: [] })
    ...
    async function load() {
      try {
        ...
        const withSprites = await Promise.all(
          stages.map(async (s) => ({ ...s, sprite: await fetchSprite(s.name) }))
        )
    ...
    async function loadDamageProfile() {
      try {
        ...
        const responses = await Promise.all(
          attackerTypes.map((type) => fetch(`https://pokeapi.co/api/v2/type/${type}/`))
        )

        if (responses.some((res) => !res.ok) || cancelled) return

        const payloads = await Promise.all(responses.map((res) => res.json()))
        const bestMultiplierByTarget = new Map(ALL_TYPES.map((type) => [type, 1]))

        function multiplierAgainst(typeData, defendingType) {
          const relations = typeData.damage_relations

          if ((relations.double_damage_to ?? []).some((item) => item.name === defendingType)) return 2
          if ((relations.half_damage_to ?? []).some((item) => item.name === defendingType)) return 0.5
          if ((relations.no_damage_to ?? []).some((item) => item.name === defendingType)) return 0
          return 1
        }
        ...
        for (const defendingType of ALL_TYPES) {
          ...
          for (const attackerTypeData of payloads) {
            const currentMultiplier = multiplierAgainst(attackerTypeData, defendingType)
        ...
        more.sort((a, b) => b.rawMultiplier - a.rawMultiplier || a.type.localeCompare(b.type))
        normal.sort((a, b) => a.type.localeCompare(b.type))
        less.sort((a, b) => a.rawMultiplier - b.rawMultiplier || a.type.localeCompare(b.type))
      ...
      <div className={styles.columns}>
        <div className={styles.topSection}>
          ...
          <div className={styles.identity}>
            <p className={styles.pokeNumber}>#{String(data.id).padStart(4, '0')}</p>
                ...
                {damageSummary.map((group) => (
                  <div key={group.key} className={styles.matchupCard}>
                    ...
                    {group.items.length > 0 ? (
                      ...
                      <div className={styles.matchupEmpty}>{group.emptyText}</div>
                    )}
                  </div>
                ))}
              </div>
